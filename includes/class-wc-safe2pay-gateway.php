...
class WC_Safe2Pay_Gateway extends WC_Payment_Gateway {
	public function __construct() {
		...
		$this->method_description = __( 'Aceite pagamentos por boleto, cartões de crédito e Pix pelo Safe2Pay.', 'woo-safe2pay' );
		...
		$this->tc_interest_rate              = $this->get_option( 'tc_interest_rate' );
	...
	public function checkout_scripts() {
		if ( $this->IsAvailable() ) {
			if ( ! get_query_var( 'order-received' ) ) {
				...
				wp_enqueue_script( 'safe2pay-checkout', plugins_url( 'assets/js/frontend/transparent-checkout.js', plugin_dir_path( __FILE__ ) ), array(
					'jquery',
					'safe2pay-library'
				), WC_SAFE2PAY_VERSION, true );
	...
	public function init_form_fields() {
		$this->form_fields = array(
			...
			'isEnablePartialPayment'        => array(
				...
				'type'        => 'checkbox',
				'description' => __( 'Aceitar pagamento parcial do boleto bancário. Será permitido para o cliente pagar entre 50% e 200% do valor total da transação.', 'woo-safe2pay' ),
				'default'     => false
			),
			'interestRate'                  => array(
				'title'       => __( 'Taxa de juros no Boleto', 'woo-safe2pay' ),
				'type'        => 'number',
			...
			'tc_interest_rate' => array(
				'title' => __('Taxa de juros por parcela', 'woo-safe2pay'),
				'type' => 'text',
				'description' => __('Taxa de juros por parcela. Insira o valor da taxa em porcentagem (ex: 1.5 para 1.5%). Separe os valores por ";" caso haja taxas diferentes para cada parcela. Insira "0" caso não queira adicionar taxa para uma determinada parcela. Ex: 0;0;0;1.5;1.5', 'woo-safe2pay'),
			...
			'invoice_prefix'                => array(
				...
				'type'        => 'text',
				'description' => __( 'Prefixo do Pedido. Se deixado em branco, a referência na Safe2Pay será o número do pedido.', 'woo-safe2pay' ),
				'desc_tip'    => true,
				'default'     => 'WC-'
			),
			'testing'                       => array(
				'title'       => __( 'Modo de teste', 'woo-safe2pay' ),
				'type'        => 'title',
				'description' => ''
			),
			'debug'                         => array(
				'title'       => __( 'Log', 'woo-safe2pay' ),
				'type'        => 'checkbox',
				'label'       => __( 'Ativar log', 'woo-safe2pay' ),
				'default'     => 'no',
				'description' => sprintf( __( 'Gravar os eventos em log %s', 'woo-safe2pay' ), $this->get_log_view() )
			)
		);
	...
	public function admin_options() {
		wp_enqueue_script( 'safe2pay-admin', plugins_url( 'assets/js/admin/admin.js', plugin_dir_path( __FILE__ ) ), array( 'jquery' ), WC_SAFE2PAY_VERSION, true );
	...
	public function payment_fields() {
		...
		wc_get_template(
			...
			array(
				...
				'tc_interest_rate'              => $this->tc_interest_rate,
				'tc_pix'                        => $this->tc_pix,
		...
		switch ( $method ) {
			case "bank-slip":
				...
				if ( $posted->BankSlipUrl != null ) {
					...
					$meta_data[ __( 'Vencimento', 'woo-safe2pay' ) ]       = sanitize_text_field( (string) $posted->DueDate );
	...
	public function update_order_status( $posted, $order_id ) {
		if ( isset( $posted->IdTransaction ) ) {
			...
			if ( $id > 0 ) {
				if ( 'yes' === $this->debug ) {
					$this->log->add( $this->id, 'Status do pedido ' . $order->get_order_number() . ' é: ' . intval( $posted->Status ) );
				}
				...
				switch ( $posted->Status ) {
					...
					case '5':
						...
						$this->send_email(
							sprintf( __( 'Pagamento para o pedido %s entrou em disputa', 'woo-safe2pay' ), $order->get_order_number() ),
							__( 'Payment in dispute', 'woo-safe2pay' ),
							sprintf( __( 'Pedido %s entrou em disputa na  Safe2Pay. Acesse seu painel da Safe2Pay para maiores detalhes.', 'woo-safe2pay' ), $order->get_order_number() )
						);
					...
					case '6':
						...
						$this->send_email(
							sprintf( __( 'Pagamento para o pedido %s devolvido', 'woo-safe2pay' ), $order->get_order_number() ),
							__( 'Pagamento devolvido', 'woo-safe2pay' ),
							sprintf( __( 'Pedido %s foi devolvido na Safe2Pay.', 'woo-safe2pay' ), $order->get_order_number() )
						);
					...
					case '7':
					case '12':
						$order->update_status( 'cancelled', __( 'Safe2Pay: Chargeback.', 'woo-safe2pay' ) );
				...
				if ( 'yes' === $this->debug ) {
					$this->log->add( $this->id, 'Error: Pedido não encontrado.' );
				}
			}
		}
	}
	...
	public function thankyou_page( $order_id ) {
		...
		if ( isset( $data['paymenttype'] ) ) {
			switch ( $data['paymenttype'] ) {
				...
				case '3':
					wc_get_template(
						'crypto-payment-instructions.php',
						[
							'paymenttype'   => $data['paymenttype'],
							'link'          => $data['link'],
